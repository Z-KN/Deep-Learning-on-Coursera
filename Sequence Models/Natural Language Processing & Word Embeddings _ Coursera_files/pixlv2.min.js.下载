...
var maKey, maEnvironment, maUtmData, maHost, sessionLength, sessionId, pixlId = 0, logging = !1; function submitCall(e, t, n, s) { const a = "eventParameters=" + encodeURIComponent(JSON.stringify(e)) + "&utmValues=" + (null === maUtmData ? "" : encodeURIComponent(JSON.stringify(maUtmData))) + "&isPageLoad=" + t + "&maCookie=" + n + "&isSessionStart=" + s; var o = ""; const i = window.location; i && (o = encodeURIComponent(i.protocol + "//" + i.host + i.pathname)); const r = maHost + "/pixl/getpixl/" + maKey + "/" + pixlId + "/" + encodeURIComponent(maEnvironment.replace(":", "|")) + "?" + a + "&originUrl=" + o + "&callback=?"; var d = new XMLHttpRequest; d.open("POST", r), d.setRequestHeader("Content-Type", "application/x-www-form-urlencoded"), d.onload = function () { if (200 === d.status) { if (d.responseText.length > 0) { var e = JSON.parse(d.responseText); e.sessionId.length > 0 && setCookie("maId", JSON.stringify({ cid: e.canvasId, sid: e.sessionId, isSidSaved: e.isSidSaved, sessionStart: e.sessionStart }), addMinutes(new Date, sessionLength)), e.canvasId.length > 0 && e.sessionId.length > 0 && "unknown" === e.canvasId && (logging && console.log("Canvas is unknown, about to call getCanvasData() with session id of " + e.sessionId + " session start of " + e.sessionStart), getCanvasData(e.sessionId, e.sessionStart, e.isSidSaved)) } logging && console.log("successful submission of session/event xhr status:" + d.status) } else 200 !== d.status && logging && console.log("Request failed to submit session/event.  Returned status of " + d.status) }, logging && console.log("Request sent to submit session/event with isSessionStart=" + s + " isPageLoad:" + t), d.send() } function submitCanvasUpdate(e, t, n) { const s = "canvasValues=" + (null === t ? "" : encodeURIComponent(JSON.stringify(t))), a = maHost + "/pixl/updatewithcanvasdata/" + maKey + "/" + pixlId + "/" + e + "?" + s + "&callback=?"; var o = new XMLHttpRequest; o.open("POST", a), o.setRequestHeader("Content-Type", "application/x-www-form-urlencoded"), o.onload = function () { if (200 === o.status) { if (o.responseText.length > 0) { var t = JSON.parse(o.responseText); t.canvasId && t.canvasId.length > 0 && (setCookie("maId", JSON.stringify({ cid: t.canvasId, sid: t.sessionId, isSidSaved: !0, sessionStart: n }), addMinutes(new Date, sessionLength)), logging && console.log("Canvas id returned as " + t.canvasId + " and added to cookie with session " + e)) } logging && console.log("Request successful to update canvas " + o.status) } else 200 !== o.status && logging && console.log("Request failed to update canvas.  Returned status of " + o.status) }, logging && console.log("Sent request to update canvas with canvas id =" + t.canvasId), o.send() } function maTrackEvent(e, t, n, s, a) { var o = { user: e, category: t, label: n, value: s, amount: a }; const i = getCookie("maId"); null !== i ? (extendCookieSession(i), submitCall(o, !1, getCookie("maId"), !1)) : (setCookie("maId", JSON.stringify({ cid: "unknown", sid: getSessionGuid(), isSidSaved: !1, sessionStart: getUtcDate() }), addMinutes(new Date, sessionLength)), submitCall(o, !1, getCookie("maId"), !0)) } function getCanvasData(e, t, n) { try { var s = { fonts: { extendedJsFonts: !1 }, excludes: { userAgent: !1 } }; window.requestIdleCallback ? window.requestIdleCallback(function () { window.Fingerprint2(s).get(function (s, a) { const o = processFingerprint(s, a); setCookie("maId", JSON.stringify({ cid: o.canvasId, sid: e, isSidSaved: n, sessionStart: t }), addMinutes(new Date, sessionLength)), submitCanvasUpdate(e, o, t) }) }) : setTimeout(function () { window.Fingerprint2(s).get(function (s, a) { const o = processFingerprint(s, a); setCookie("maId", JSON.stringify({ cid: o.canvasId, sid: e, isSidSaved: n, sessionStart: t }), addMinutes(new Date, sessionLength)), submitCanvasUpdate(e, o, t) }) }, 500) } catch (e) { logging && console.log("Error getting canvas: " + e) } } function getParameterByName(e) { e = e.replace(/[\[]/, "\\[").replace(/[\]]/, "\\]"); const t = new RegExp("[\\?&]" + e + "=([^&#]*)").exec(location.search); return null === t ? "" : decodeURIComponent(t[1].replace(/\+/g, " ")) } function processFingerprint(e, t) { var n = { canvasId: e, adblock: null, timezoneOffset: 0, touchPoints: 0, touchEvent: null, touchStart: null, platform: "unknown", resolution: "unknown" }; for (window.i in t) if (t.hasOwnProperty(window.i)) { const e = t[window.i].value; switch (t[window.i].key) { case "adblock": n.adblock = e; break; case "timezone_offset": n.timezoneOffset = e / 60 * -1; break; case "touch_support": n.touchPoints = e[0], n.touchEvent = e[1], n.touchStart = e[2]; break; case "navigator_platform": "unknown" === n.platform && (n.platform = e); break; case "user_agent": -1 === e.indexOf("WOW64") && -1 === e.indexOf("Win64") || (n.platform = "Win64"); break; case "resolution": n.resolution = e[0] + "x" + e[1] } } return n } function setCookie(e, t, n) { var s = "; expires=" + n.toUTCString(); document.cookie = e + "=" + (t || "") + s + "; path=/" } function getCookie(e) { for (var t = e + "=", n = document.cookie.split(";"), s = 0; s < n.length; s++) { for (var a = n[s]; " " === a.charAt(0);)a = a.substring(1, a.length); if (0 === a.indexOf(t)) return a.substring(t.length, a.length) } return null } function eraseCookie(e) { document.cookie = e + "=; Max-Age=-99999999;" } function extendCookieSession(e) { var t = JSON.parse(e); logging && console.log("extending cookie session length: " + t.sid + " cid length: " + t.cid.length), setCookie("maId", JSON.stringify({ cid: t.cid, sid: t.sid, isSidSaved: t.isSidSaved, sessionStart: t.sessionStart }), addMinutes(new Date, sessionLength)) } function addMinutes(e, t) { return new Date(e.getTime() + 6e4 * t) } function getSessionGuid() { return null !== sessionId && "" !== sessionId && void 0 !== sessionId ? sessionId : sessionId = ([1e7] + -1e3 + -4e3 + -8e3 + -1e11).replace(/[018]/g, e => (e ^ crypto.getRandomValues(new Uint8Array(1))[0] & 15 >> e / 4).toString(16)) } function getUtcDate() { var e = new Date, t = Date.UTC(e.getUTCFullYear(), e.getUTCMonth(), e.getUTCDate(), e.getUTCHours(), e.getUTCMinutes(), e.getUTCSeconds()); return new Date(t) } !function () { !function (e) { const t = document.getElementById("abbot-script"); var n, s, a, o, i; sessionLength = 30; for (let e = 0; e < t.attributes.length; e++) { const d = t.attributes[e]; if (d.specified && -1 !== d.name.indexOf("data-")) if ("data-env" === d.name) maEnvironment = d.value; else if ("data-wid" === d.name) pixlId = d.value; else if ("data-pid" === d.name) pixlId = d.value; else if ("data-key" === d.name) maKey = d.value; else if ("data-label" === d.name) n = d.value; else if ("data-value" === d.name) s = d.value; else if ("data-category" === d.name) a = d.value; else if ("data-userid" === d.name) o = d.value; else if ("data-user" === d.name) o = d.value; else if ("data-amount" === d.name) i = d.value; else if ("data-session" === d.name) { var r = parseInt(d.value); null === r || isNaN(r) || (sessionLength = r) } } var d = { label: n, value: s, category: a, user: o, amount: i }; if (null == maEnvironment || null === maKey || 0 === pixlId) return logging && (console.log("maEnvironment:" + maEnvironment), console.log("maKey:" + maKey), console.log("pixlId:" + pixlId), console.log("Your setup is missing a required parameter.")), !1; maHost = "https://" + maEnvironment, "prod" === maEnvironment ? maHost = "https://mapixl.com" : "qa" === maEnvironment ? maHost = "https://qa.mapixl.com" : "dev" === maEnvironment && (maHost = "https://localhost:44317"); var l = ""; if (document.referrer) { var u = new URL(document.referrer); l = u.protocol + "//" + u.host + u.pathname } maUtmData = { utmSource: getParameterByName("utm_source"), utmMedium: getParameterByName("utm_medium"), utmCampaign: getParameterByName("utm_campaign"), utmTerm: getParameterByName("utm_term"), utmContent: getParameterByName("utm_content"), isGoogleAd: !!getParameterByName("gclid"), refererUrl: l }, null !== e ? (extendCookieSession(e), submitCall(d, !0, getCookie("maId"), !1)) : (setCookie("maId", JSON.stringify({ cid: "unknown", sid: getSessionGuid(), isSidSaved: !1, sessionStart: getUtcDate() }), addMinutes(new Date, sessionLength)), submitCall(d, !0, getCookie("maId"), !0)) }(getCookie("maId")) }();